...
from app.models import Patient, Nurse
...
# Check for undispensed drugs and send an email notification to current logged user
def undispensed_drugs_check():
    ...
    for patient in patients:
        for assoc in patient.drugs:
            if (assoc.dispensed == 0):
                ...
                drug_time = drug_time.replace(year=time.year, month=time.month, day=time.day)
                if (time > drug_time):
                    delta = time - drug_time
                    if (delta.seconds > (CHECK_PERIOD * 60) and delta.seconds < (CHECK_PERIOD * 4 * 60)):  # 15 to 60 minutes after drug time
                        undispensed_drugs.append({'patient_id': patient.patient_id,
    ...
    if (len(undispensed_drugs) > 0):
        email_notification("DispensED notification message", [Config_Disp.MAIL_DEFAULT_RECIPIENT], undispensed_drugs)
...
mail_handler = SMTPHandler((Config_Disp.MAIL_SERVER, Config_Disp.MAIL_PORT),
                           'no-reply@' + Config_Disp.MAIL_SERVER, [Config_Disp.MAIL_DEFAULT_SENDER],
                           'DispensED notification', credentials)
